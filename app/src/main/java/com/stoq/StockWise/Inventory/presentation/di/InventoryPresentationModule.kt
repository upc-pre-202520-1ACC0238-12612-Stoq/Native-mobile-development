...
val inventoryPresentationModule = module {
    ...
    single<InventoryApiService> { 
        NetworkClient.createInventoryApiService(get<JwtRepository>()) 
    }
    single<InventoryRepository> { 
        InventoryRepositoryImpl(get<InventoryApiService>()) 
    }
    viewModel { 
        InventorySetupViewModel(get<InventoryRepository>()) 
    }
}
